...
def get_harmonic_level(number, params, max_level=0.999999): 
    a_max = 1.0
    a = compute_harmonic(number, params)
    v = math.log(abs(a / a_max))
    level = float(max_level) + 8.0 * v
    print('DEBUG n = {0}, a = {1}, v = {2}'.format(number, a, v))
    if level < 0:
        return 0.0
    return level

def get_harmonic_levels(waveform_name, count, max_level=0.999999):
...
if __name__ == '__main__':
    ...
    levels = get_harmonic_levels('square', 64)
    for i, level in enumerate(levels):
        print('Sine {0} {1}'.format(i + 1, level))
